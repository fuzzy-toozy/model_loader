...
public:
    ...
    void TakeScreenshot(const std::filesystem::path& path);
    ...
    inline void SetScreen(const GLfloat width, const GLfloat height)
    {
        m_screen_w = width;
        m_screen_h = height;
        m_aspect = (GLfloat)m_screen_w / m_screen_h;
        m_proj_transform = glm::perspective(glm::radians(m_fov), m_aspect, m_z_near, m_z_far);
    }
    ...
    inline void SetFov(const GLfloat fov) { m_fov = fov; m_proj_transform = glm::perspective(glm::radians(m_fov), m_aspect, m_z_near, m_z_far); }
    ...
    inline auto GetAspect() const { return m_aspect; }
    inline auto GetZnear() const { return m_z_near; }
    inline auto GetZfar() const { return m_z_far; }
...
private:
    ...
    inline bool InitMouseCursor(const GLfloat x_pos, const GLfloat y_pos)
    {
        ...
        std::cout << "INITIAL CURSOR POS X: " << x_pos << std::endl;
        std::cout << "INITIAL CURSOR POS Y: " << y_pos << std::endl;
        return true;
    }
...
;
    ...
    GLint m_screen_w{800};
    GLint m_screen_h{600};
    GLfloat m_z_near{1.f};
    GLfloat m_z_far{100.f};
